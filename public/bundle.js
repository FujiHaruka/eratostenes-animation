...
function calcLinearColor(_ref2) {
  var from = _ref2.from,
      to = _ref2.to,
      progress = _ref2.progress;

  var srcColor = color(from).array();
  var destColor = color(to).array();
  var indexes = [0, 1, 2];
  var resultColor = color(indexes.map(function (i) {
    return calcLinear({ from: srcColor[i], to: destColor[i], progress: progress });
  }));
  return resultColor.rgbNumber();
}

function calcBoxPosition(_ref3) {
  var number = _ref3.number,
      size = _ref3.size,
      cols = _ref3.cols;

  var xLeft = (APP_WIDTH - size * cols) / 2;
  var x = xLeft + size * ((number - 1) % cols);
  var y = size * Math.floor((number - 1) / cols);
  return { x: x, y: y };
}

function calcBoxSize(cols) {
...
var Animation = function () {
  function Animation(_ref) {
    var frameAction = _ref.frameAction,
        frames = _ref.frames;
  ...
  createClass(Animation, [{
    ...
    value: function nextFrame() {
      var frameAction = this.frameAction,
          frames = this.frames;

      if (this.currentFrame < frames) {
        this.currentFrame += 1;
        var progress = this.currentFrame / frames;
        return function () {
...
var Box = function () {
  ...
  createClass(Box, [{
    ...
    value: function getNextActions() {
      ...
      try {
        for (var _iterator = animations[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          var action = animation.nextFrame();
          if (action) {
            actions.push(action);
          }
        }
    ...
    key: 'setMovingAnimation',
    value: function setMovingAnimation(_ref) {
      var x = _ref.x,
          y = _ref.y,
          frames = _ref.frames;
      ...
      this.movingAnimation = new Animation({
        ...
        frameAction: function frameAction(progress) {
          graphic.x = calcLinear({ from: origX, to: x, progress: progress });
          graphic.y = calcLinear({ from: origY, to: y, progress: progress });
        }
      });
    }
  }, {
    key: 'setColorAction',
    value: function setColorAction(_ref2) {
      var destColor = _ref2.color,
          frames = _ref2.frames;

      var s = this;
      var graphic = s.graphic,
          color = s.color;

      this.colorAnimation = new Animation({
        frames: frames,
        frameAction: function frameAction(progress) {
          var nextColor = calcLinearColor({ from: color, to: destColor, progress: progress });
    ...
    key: 'setResizeAnimation',
    value: function setResizeAnimation(_ref3) {
      var destSize = _ref3.size,
          frames = _ref3.frames;
...
var BoxContainer = function () {
  function BoxContainer(boxCount) {
    var _this = this;

    classCallCheck(this, BoxContainer);

    this.currentPrimeNumber = 2;
    var size = calcBoxSize(this.currentPrimeNumber);
    this.boxes = new Array(boxCount).fill(null).map(function (_, i) {
      var number = i + 1;

      var _calcBoxPosition = calcBoxPosition({
        number: number,
        cols: _this.currentPrimeNumber,
        size: size
      }),
  ...
  createClass(BoxContainer, [{
    ...
    value: function getNextActions() {
      ...
      try {
        for (var _iterator = this.boxes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          var actions = box.getNextActions();
    ...
    key: 'setArrangementAnimation',
    value: function setArrangementAnimation(_ref) {
      var primeNumber = _ref.primeNumber,
          frames = _ref.frames;
      ...
      try {
        for (var _iterator2 = this.boxes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          var size = calcBoxSize(primeNumber);

          var _calcBoxPosition2 = calcBoxPosition({
            number: box.number,
            cols: primeNumber,
            size: size
          }),
          ...
          box.setMovingAnimation({ x: x, y: y, frames: frames });
          if (size !== box.size) {
            box.setResizeAnimation({ size: size, frames: frames });
          }
        }
      } catch (err) {
    ...
    key: 'setMarkAnimation',
    value: function setMarkAnimation(_ref2) {
...
var Application = function (_PIXI$Application) {
  inherits(Application, _PIXI$Application);
  ...
  createClass(Application, [{
    ...
    key: "addBoxContainer",
    value: function addBoxContainer(boxContainer) {
      var _iteratorNormalCompletion = true;
      var _didIteratorError = false;
      var _iteratorError = undefined;

      try {
        for (var _iterator = boxContainer.boxes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var box = _step.value;
...
var TaskCreator = function () {
  function TaskCreator() {
    classCallCheck(this, TaskCreator);

    this.queue = [];
  }

  createClass(TaskCreator, [{
    key: "add",
...
var asleep = function asleep(sleepMs) {
  var res;
  setTimeout(function () {
    res();
  }, sleepMs);
...
async function main(pixiView) {
  ...
  try {
    for (var _iterator = schedule[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var setAnimation = _step.value;

      setAnimation();
      while (true) {
        var actions = boxContainer.getNextActions();
        if (actions.length === 0) {
          break;
        }
        Task.add(actions);
...
window.addEventListener('DOMContentLoaded', function () {
  var pixiView = document.getElementById('pixi-view');
  main(pixiView);
});
